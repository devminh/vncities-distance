...
func GetCityInfo2(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Context-Type", "application/x-www-form-urlencoded")
	w.Header().Set("Access-Control-Allow-Origin", "*")
	params := mux.Vars(r)
...
func getCity2(cityName string) ([]models.City, error) {
	// create the postgres db connection
	db := createConnection()
	...
	// create the select sql query
	sqlStatement := `SELECT * FROM vn_cities WHERE city LIKE $1`
